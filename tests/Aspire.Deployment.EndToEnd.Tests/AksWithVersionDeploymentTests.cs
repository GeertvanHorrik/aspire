...
/// <summary>
/// End-to-end tests for deploying Aspire applications to AKS with a specific Kubernetes version.
/// Verifies that <c>.WithVersion("1.30")</c> correctly provisions the cluster at the requested version.
/// </summary>
public sealed class AksWithVersionDeploymentTests(ITestOutputHelper output)
{
    private static readonly TimeSpan s_testTimeout = TimeSpan.FromMinutes(45);

    [Fact]
    public async Task DeployStarterToAksWithSpecificVersion()
    {
        ...
        await DeployStarterToAksWithSpecificVersionCore(cancellationToken);
    }

    private async Task DeployStarterToAksWithSpecificVersionCore(CancellationToken cancellationToken)
    {
        ...
        var resourceGroupName = DeploymentE2ETestHelpers.GenerateResourceGroupName("aks-version");
        var projectName = "AksVersion";

        output.WriteLine($"Test: {nameof(DeployStarterToAksWithSpecificVersion)}");
        ...
        try
        {
            ...
            // Step 6: Modify AppHost.cs to add AKS environment with specific version
...
""";
            ...
            output.WriteLine("Modified AppHost.cs with AddAzureKubernetesEnvironment and WithVersion(\"1.30\")");
            ...
            // Step 13: Verify Kubernetes version contains 1.30
            output.WriteLine("Step 13: Verifying Kubernetes version...");
            await auto.TypeAsync("kubectl version --short 2>/dev/null || kubectl version");
            await auto.EnterAsync();
            await auto.WaitUntilTextAsync("1.30", timeout: TimeSpan.FromSeconds(30));
            ...
            output.WriteLine($"AKS versioned deployment completed in {duration}");
            ...
            output.WriteLine("✅ Test passed - Aspire app deployed to AKS with specific version 1.30!");
        }
        catch (Exception ex)
        {
            var duration = DateTime.UtcNow - startTime;
