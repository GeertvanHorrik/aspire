    ...
    public DistributedApplicationPipeline()
    {
        ...
        _steps.Add(new PipelineStep
        {
            ...
            Action = async context =>
            {
                ...
                // In interactive mode, give the user a chance to start the runtime
                var interactionService = context.Services.GetService<IInteractionService>();
                if (interactionService?.IsAvailable == true)
                {
                    while (!await containerRuntime.CheckIfRunningAsync(context.CancellationToken).ConfigureAwait(false))
                    {
                        var result = await interactionService.PromptConfirmationAsync(
                            $"{containerRuntime.Name} is not running",
                            $"Start {containerRuntime.Name} and confirm to continue, or cancel to abort.",
